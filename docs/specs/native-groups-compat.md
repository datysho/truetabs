...
1. Same-title reuse invariant: before `createOurGroup` with title T in window W, adopt an existing live group in W titled T when it is (a) already ours, or (b) unclaimed-foreign with OUR signature match (chip-restored copies of our own groups re-enter management instead of spawning "Google" #2). Hand-made foreign groups with alien signatures stay untouched. This kills the twin-group source live; enforcement sits inside the single group-creation choke point. *G4 amendments: reuse requires the same KIND (site/topic/rule/Other and the domain for site groups) - "News" the theme never swallows news-site tabs; a kind-mismatched site creation falls back to the full domain as its title (the registered-collision rule extended to live groups); zero successful joins returns null rather than minting a same-title group beside its twin; and an adopted twin reports `meta.adopted` so Organize bookkeeping (`createdGids`, the undo slot, the "created" counter) never treats a pre-existing group as this run's creation - undo must not dissolve what it did not create.*
