...
| 3f | **Bookmark groups** (opt-in) | A folder under "TrueTabs" in your bookmarks is a durable group definition: **Open** materializes it (already-open tabs join instead of duplicating), **Update folder** pushes the live membership back, and the popup marks a group that drifted from its folder with a dot. Closing tabs never touches bookmarks - the folder is the recipe, the group is tonight's cooking. Definitions travel between browsers through Chrome's own bookmark sync. Uses the optional bookmarks permission, asked only when you switch it on. |
