...
- A group label is never compared as a raw string: the model spells one topic "YouTube" this run and "Youtube" the next, and the group registry lives in `storage.session` - empty after every browser start while the groups it described are still on the strip. `labelKey()` is the only judge of "same label", and "which topics exist" is read from `chrome.tabGroups.query` after a `readoptGroups()` pass, never from the registry alone (lesson ai-label-drift-and-forgotten-groups; guarded by an e2e contract).
